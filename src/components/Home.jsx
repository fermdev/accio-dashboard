...
function FaqItem({ question, answer, isOpen, onToggle }) {
  return (
    <div className="border-b border-white/10 last:border-b-0">
      ...
      >
        <span className="text-white font-medium text-sm md:text-base group-hover:text-primary transition-colors">
          {question}
        </span>
        <span
          className={`material-symbols-outlined text-slate-500 shrink-0 transition-transform duration-200 ${
      ...
      >
        <div className="overflow-hidden">
          <p className="text-slate-400 text-sm leading-relaxed text-left pr-8">{answer}</p>
        </div>
      </div>
    </div>
  );
...
const Home = ({ onExplore, onNavigate }) => {
  ...
  return (
    ...
    <div
      ref={scrollRef}
      className="home-page flex-1 overflow-y-auto no-scrollbar bg-[#131314] relative"
    >
      <HomeFloatingLogos />
      <div
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-12 md:py-20 flex flex-col items-center text-center">
        <ScrollReveal direction="up" delay={0}>
          <div className="size-20 md:size-24 rounded-2xl overflow-hidden mb-8 shadow-2xl shadow-primary/20 ring-1 ring-white/10 mx-auto">
          ...
          {FEATURES.map((f, i) => (
            ...
            >
              <button
                ...
                className="w-full rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur-sm p-5 text-left hover:border-primary/40 hover:bg-white/[0.08] transition-all"
              >
        ...
        <section className="w-full mt-16 md:mt-20 text-left">
          ...
          <ScrollReveal direction="up" delay={180}>
            <div className="rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur-sm px-5 md:px-6">
